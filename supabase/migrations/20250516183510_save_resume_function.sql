-- Create RPC function to save resume with all changes atomically
...
GRANT EXECUTE ON FUNCTION public.save_resume_complete(UUID, TEXT, INT, UUID[], TEXT[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.save_resume_complete(UUID, TEXT, INT, UUID[], TEXT[]) TO service_role;

COMMENT ON FUNCTION public.save_resume_complete(UUID, TEXT, INT, UUID[], TEXT[]) IS 
...
2. Resets all suggestions in resume_suggestions and then sets specified ones to applied
3. Resets all keywords in resume_keywords and then sets specified ones to applied
