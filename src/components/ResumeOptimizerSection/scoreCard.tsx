/**
 * Enhanced ScoreCard Component - OPTIMIZED VERSION WITH ANTI-DUPLICATE PROTECTION
 *
 ...
 *
 * NEW OPTIMIZATIONS:
 * - Anti-duplicate protection to prevent excessive re-renders
 * - Debouncing for score updates to avoid cascade logging
 ...
 *
 * OPTIMIZED VERSION: Now includes anti-duplicate protection and debouncing
 * to prevent excessive re-renders and logging when suggestions/keywords are applied.
 */
const ScoreCardComponent: React.FC<ScoreCardProps> = ({
	...
	// =========================================================================
	// NEW: Anti-duplicate and debouncing protection refs
	// =========================================================================

	// Track last update time to prevent too frequent updates
	const lastUpdateTimeRef = useRef<number>(0);
	// Debounce timeout for score updates to prevent cascade logging
	const debounceTimeoutRef = useRef<NodeJS.Timeout | null>(null);
	// Track last processed score to detect actual changes
	const lastProcessedScoreRef = useRef<number>(
		optimizationScore || initialScore || 65
	);
	...
	// Debounce threshold in milliseconds to prevent excessive updates
	const DEBOUNCE_THRESHOLD = 50; // 50ms debounce
	// Score change threshold to consider it a real change (prevents micro-changes)
	const SCORE_CHANGE_THRESHOLD = 0.1; // 0.1 point minimum change
	...
	const processedScore = useMemo(() => {
		...
		// Log incoming score for debugging (only when score actually changes)
		const hasScoreChanged =
			Math.abs((optimizationScore || 0) - lastProcessedScoreRef.current) >
		...
		// Update last processed score reference
		lastProcessedScoreRef.current = finalScore || 0;
	...
	/**
	 * OPTIMIZED: Handle score changes and animate transitions with anti-duplicate protection
	 * NEW FEATURES:
	 * - Debouncing to prevent cascade logging
	 * - Score change threshold to ignore micro-changes
	 * - Time-based protection against rapid updates
	...
	useEffect(() => {
		...
		// PROTECTION 1: Check for meaningful score changes
		const scoreHasChanged =
			Math.abs(currentScore - previousScore) > SCORE_CHANGE_THRESHOLD;

		if (!scoreHasChanged || isNaN(currentScore)) {
			// Score hasn't changed meaningfully, skip update
			return;
		}

		// PROTECTION 2: Time-based debouncing to prevent rapid updates
		const now = Date.now();
		const timeSinceLastUpdate = now - lastUpdateTimeRef.current;

		if (timeSinceLastUpdate < DEBOUNCE_THRESHOLD) {
			// Too soon since last update, debounce this one
		...
		// IMMEDIATE UPDATE: Enough time has passed, update immediately
		console.log(
			`ScoreCard: Score update received: ${currentScore} (previous: ${previousScore})`
		);
	 ...
	 * OPTIMIZED: Now includes debouncing to prevent excessive updates
	 */
	useEffect(() => {
		// Skip if score is being calculated
		if (isCalculating) {
	...
	/**
	 * ENHANCED: Clean up animations and timeouts on component unmount
	 * NEW: Now includes cleanup for debounce timeout
	 */
	useEffect(() => {
		return () => {
...
/**
 * OPTIMIZED: Wrap component with React.memo and custom comparison function
 * This prevents unnecessary re-renders when props haven't meaningfully changed
 */
const ScoreCard = React.memo<ScoreCardProps>(
	ScoreCardComponent,
	(prevProps, nextProps) => {
		// CUSTOM COMPARISON: Check if props have meaningfully changed

		// Compare scores with tolerance to prevent micro-change re-renders
		const scoreUnchanged =
			Math.abs(
				(prevProps.optimizationScore || 0) - (nextProps.optimizationScore || 0)
			) < 0.1; // 0.1 point tolerance

		// Compare other important props
		const otherPropsUnchanged =
			prevProps.suggestionsApplied === nextProps.suggestionsApplied &&
			prevProps.keywordsApplied === nextProps.keywordsApplied &&
			prevProps.isCalculating === nextProps.isCalculating &&
			prevProps.showDetails === nextProps.showDetails &&
			prevProps.initialScore === nextProps.initialScore &&
			prevProps.potentialScore === nextProps.potentialScore;

		// Shallow compare for scoreBreakdown (complex object)
		const breakdownUnchanged =
			JSON.stringify(prevProps.scoreBreakdown) ===
			JSON.stringify(nextProps.scoreBreakdown);

		// Compare resume content length instead of full content for performance
		const contentLengthUnchanged =
			(prevProps.resumeContent?.length || 0) ===
			(nextProps.resumeContent?.length || 0);

		// Determine if we should skip re-render
		const shouldSkipRerender =
			scoreUnchanged &&
			otherPropsUnchanged &&
			breakdownUnchanged &&
			contentLengthUnchanged;

		// Log when we're preventing unnecessary re-renders (development only)
		if (process.env.NODE_ENV === "development" && shouldSkipRerender) {
			console.log("ScoreCard: Props unchanged, preventing re-render");
		}
