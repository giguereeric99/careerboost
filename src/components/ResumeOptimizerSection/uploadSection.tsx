 ...
 * - Resume structure validation to ensure proper CV format
...
const UploadSection: React.FC<UploadSectionProps> = ({
  ...
  const validateResumeContent = (
    ...
    try {
      ...
      if (!content || content.length < 50) {
        return {
          success: false,
          message: `Please enter at least 50 characters. Current: ${
            content?.length || 0
          } characters.`,
        };
      }
  ...
  const handleFileDrop = async (file: File) => {
    ...
    try {
      ...
      await handleResumeOptimization(fileUrl, file.name, file.size, file.type);
    } catch (err: any) {
                    ...
                    isUploadingFile ? (
                      ...
                      <UploadButton
                        className={`custom-btn ut-button:bg-blue-600 ut-button:text-white ut-button:rounded-md ut-button:px-4 ut-button:py-2 ut-button:hover:hover:bg-blue-700 ${
                          shouldDisableUpload()
                            ? "ut-button:opacity-50 ut-button:cursor-not-allowed"
                            : ""
                        }`}
                        ...
                        onBeforeUploadBegin={(files) => {
                          console.log(
                            "Upload about to begin with files:",
                            files
                          );
                        ...
                        onClientUploadComplete={(results) => {
                          ...
                          // Keep active state while processing the file
                          handleButtonUpload(results);
                          // Don't reset setIsActiveUpload here - keep it active during processing
                        }}
                        onUploadError={(error) => {
              ...
              {uploadedInfo && (
                <div className="mt-2 text-sm text-muted-foreground animate-fade-in-up text-center">
                  ...
                  <div className="text-xs text-gray-500">
                    ({readableSize(uploadedInfo.size)},{" "}
                    {mimeLabelMap[uploadedInfo.type] || uploadedInfo.type})
                  </div>
                  {/* Processing status */}
