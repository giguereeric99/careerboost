...
const SuggestionsList: React.FC<SuggestionsListProps> = ({
	...
	const cumulativeImpact = useMemo<CumulativeImpact | null>(() => {
		...
		// Calculate total point impact from applied suggestions
		const totalPointImpact = appliedSuggestions.reduce(
			(sum, suggestion) => sum + (suggestion.pointImpact || 2), // Default 2 points per suggestion
			0
		);
	...
	const handleApplySuggestion = useCallback(
		(index: number) => {
			...
			console.log("🎯 APPLY DEBUG: Before applying suggestion:", {
				suggestionIndex: index,
				suggestionId: suggestions[index]?.id,
				currentIsApplied: suggestions[index]?.isApplied,
				newIsApplied: !suggestions[index]?.isApplied,
			});
			if (isEditing) {
