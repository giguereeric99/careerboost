 ...
 *
 * MINIMAL CHANGES: Only modified to work with new standardized header structure
 */
...
export function convertTextToHTML(text: string): string {
  ...
  lines.forEach((line, index) => {
    ...
    // Handle sub-headers or job titles
      ...
      (currentSection === "resume-experience" && trimmedLine.includes("|"))
    ) {
      html += `<h3>${trimmedLine}</h3>`;
    }
    // Handle list items
...
export function applyDefaultStyling(html: string): string {
  ...
  try {
    ...
    let styledHtml = html
      ...
      .replace(
        /<h2[^>]*>/g,
        '<h2 class="text-xl font-semibold text-gray-700 mb-2 mt-4 uppercase border-b pb-1">'
      )
      .replace(
        /<h3[^>]*>/g,
 ...
 * ENHANCED: Now uses validation to prevent invalid entries
 *
...
function enhanceHeaderContactInfo(headerSection: Element): void {
  ...
  paragraphs.forEach((p) => {
    ...
    // 4. ENHANCED: Add portfolio/website spans with validation
    const portfolioRegex = /(Portfolio\s*[:;]\s*[^<|/\n]+)/gi;
    if (portfolioRegex.test(newHTML)) {
      newHTML = newHTML.replace(portfolioRegex, (match) => {
        // Only add span if valid, otherwise remove
        if (isValidContact(match, "portfolio")) {
          return `<span class="portfolio">${match}</span>`;
        } else {
          console.log("Removing invalid portfolio entry:", match);
          return "";
        }
      });
      textContentUpdated = true;
    ...
    // 5. ENHANCED: Add LinkedIn spans with validation
    const linkedinRegex = /(LinkedIn\s*[:;]\s*[^<|/\n]+)/gi;
    if (linkedinRegex.test(newHTML)) {
      newHTML = newHTML.replace(linkedinRegex, (match) => {
        // Only add span if valid, otherwise remove
        if (isValidContact(match, "linkedin")) {
          return `<span class="linkedin">${match}</span>`;
        } else {
          console.log("Removing invalid LinkedIn entry:", match);
          return "";
        }
      });
      textContentUpdated = true;
      ...
      parts.forEach((part) => {
        if (part === "/" || part === "|") {
          ...
          if (trimmedPart) {
            ...
            if (
              trimmedPart.toLowerCase().includes("portfolio") ||
              trimmedPart.toLowerCase().includes("website")
            ) {
              // Only add if valid
              if (isValidContact(trimmedPart, "portfolio")) {
                processedHTML += `<span class="portfolio">${trimmedPart}</span>`;
                textContentUpdated = true;
              }
            } else if (
              trimmedPart.toLowerCase().includes("linkedin") ||
              trimmedPart.toLowerCase().includes("github")
            ) {
              // Only add if valid
              if (isValidContact(trimmedPart, "linkedin")) {
                processedHTML += `<span class="linkedin">${trimmedPart}</span>`;
                textContentUpdated = true;
              }
            } else {
    ...
    // ENHANCED: Clean up separators after removals
    if (textContentUpdated) {
      newHTML = newHTML.replace(/\s*\|\s*<\/p>/g, "</p>"); // Remove trailing separators
      newHTML = newHTML.replace(/\|\s*\|/g, "|"); // Remove double separators
      newHTML = newHTML.replace(/\s+\|$/gm, ""); // Remove trailing separators at end of lines
      p.innerHTML = newHTML;
    }
  });
}
 ...
 * ENHANCED: Now generates standardized header structure
 *
...
export function prepareOptimizedTextForEditor(optimizedText: string): string {
  ...
  try {
    ...
    // Step 2: ENHANCED header processing with standardization
    const headerSection = doc.querySelector("section#resume-header");
    if (headerSection) {
      ...
      // Enhanced processing of contact information with validation
      enhanceHeaderContactInfo(headerSection);

      // ENHANCED: Generate standardized header structure
      try {
        const headerInfo = extractHeaderInfo(headerSection.outerHTML);
        const standardizedHeader = generateStandardizedHeader(headerInfo);
        headerSection.outerHTML = standardizedHeader;
        console.log("Header standardized in prepareOptimizedTextForEditor");
      } catch (error) {
        console.error("Error standardizing header:", error);
      }
    }
    ...
    try {
      ...
      // ENHANCED: Process Portfolio and LinkedIn with validation
      processedHtml = processedHtml.replace(
        /(Portfolio\s*[:;]\s*[^<|/\n]+)/gi,
        (match) => {
          // Only add span if not just "Portfolio" or "Portfolio:"
          if (
            match.toLowerCase() !== "portfolio" &&
            match.toLowerCase() !== "portfolio:"
          ) {
            return `<span class="portfolio">${match}</span>`;
          }
          return ""; // Remove invalid entries
        }
      );
      ...
      processedHtml = processedHtml.replace(
        /(LinkedIn\s*[:;]\s*[^<|/\n]+)/gi,
        (match) => {
          // Only add span if not just "LinkedIn" or "LinkedIn:"
          if (
            match.toLowerCase() !== "linkedin" &&
            match.toLowerCase() !== "linkedin:"
          ) {
            return `<span class="linkedin">${match}</span>`;
          }
      ...
      // Clean up separators after removals
      processedHtml = processedHtml.replace(/\s*\|\s*<\/p>/g, "</p>");
